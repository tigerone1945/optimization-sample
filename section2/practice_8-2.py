...
for g1 in group_list:
    for g2 in group_list:
        if g1 != g2:
            prob += (lpSum([x[m, g1] * viscosity_dict[m] for m in material_list]) * min_group_size >= 
                     lpSum([x[m, g2] * viscosity_dict[m] for m in material_list]) * max_group_size - tolerance * max_group_size * min_group_size)
            prob += (lpSum([x[m, g1] * viscosity_dict[m] for m in material_list]) * min_group_size <= 
                     lpSum([x[m, g2] * viscosity_dict[m] for m in material_list]) * max_group_size + tolerance * max_group_size * min_group_size)
...
#     for g2 in group_list:
#         if g1 != g2:
#             prob += (lpSum([x[m, g1] * viscosity_dict[m] for m in material_list])  >= 
#                      lpSum([x[m, g2] * viscosity_dict[m] for m in material_list]) - tolerance * group_size)
#             prob += (lpSum([x[m, g1] * viscosity_dict[m] for m in material_list])  <= 
#                      lpSum([x[m, g2] * viscosity_dict[m] for m in material_list]) + tolerance * group_size)
